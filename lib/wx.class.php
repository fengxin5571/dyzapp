...
class Wx{
    ...
    private $selfuuid="";
    private $selfhttps_header;
    private $selfpost_url_header;
    public $error;
    /**
     * @return the $selfuuid
     */
    public function getSelfuuid()
    {
        return $this->selfuuid;
    }

    /**
     * @param string $selfuuid
     */
    public function setSelfuuid($selfuuid)
    {
    ...
    public function get_uri($uuid)
    {
        ...
        $this->selfhttps_header=$https_header;
        $post_url_header = $https_header . "/cgi-bin/mmwebwx-bin";
        $this->selfpost_url_header=$post_url_header;
    ...
    public function wxinit($post)
    {
        ...
        $url = $this->selfhttps_header . '/cgi-bin/mmwebwx-bin/webwxinit?pass_ticket=' . $post->pass_ticket . '&skey=' . $post->skey . '&r=' . time();
     ...
     * @return array $data
     */
    public function webwxgetcontact($post, $post_url_header='1')
    {

        $url = $this->selfpost_url_header . '/webwxgetcontact?pass_ticket=' . $post->pass_ticket . '&seq=0&skey=' . $post->skey . '&r=' . time();
     ...
     */
    public
    function webwxsendmsg($post, $init_obj, $to, $word)
    {

        $url = $this->selfpost_url_header . '/webwxsendmsg?lang=zh_CN&pass_ticket=' . $post->pass_ticket;
        ...
        $params = array(
            ...
            'Msg' => array(
                "Type" => 1,
                "Content" => $word,
                "FromUserName" => $init_obj->UserName,
                "ToUserName" => $to,
                "LocalID" => $clientMsgId,
     ...
     * @return bool
     */
    public function wxloginout($post, $post_url_header='1')
    {
        $url = $this->selfpost_url_header. '/webwxlogout?redirect=1&type=1&skey=' . urlencode($post->skey);
        $param = array(
            'sid' => $post->sid,
            'uin' => $post->uin,
        );
